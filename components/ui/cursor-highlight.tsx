...
export function CursorHighlight({
  ...
  const directionStyles: Record<
    ...
    {
      initial: { membrePath: string };
      animate: { membrePath: string };
      transition: { duration: number; ease: "easeInOut"; delay: number };
    }
  > = {
    left: {
      initial: { membrePath: "inset(0 100% 0 0)" },
      animate: {
        membrePath: isInView ? "inset(0 0% 0 0)" : "inset(0 100% 0 0)",
      },
      transition: { duration, ease: "easeInOut", delay },
    },
    right: {
      initial: { membrePath: "inset(0 0 0 100%)" },
      animate: {
        membrePath: isInView ? "inset(0 0 0 0%)" : "inset(0 0 0 100%)",
      },
      transition: { duration, ease: "easeInOut", delay },
    },
    top: {
      initial: { membrePath: "inset(100% 0 0 0)" },
      animate: {
        membrePath: isInView ? "inset(0% 0 0 0)" : "inset(100% 0 0 0)",
      },
      transition: { duration, ease: "easeInOut", delay },
    },
    bottom: {
      initial: { membrePath: "inset(0 0 100% 0)" },
      animate: {
        membrePath: isInView ? "inset(0 0 0% 0)" : "inset(0 0 100% 0)",
      },
      transition: { duration, ease: "easeInOut", delay },
    },
  };
    ...
    >
      ...
      <div className="relative z-40">
        ...
        <motion.span
          key={resetKey}
          className={cn(
            "absolute inset-0 bg-membre-text text-transparent z-30",
            gradientClass,
            className
          )}
