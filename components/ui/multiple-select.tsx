    ...
    <AnimatePresence mode={'popLayout'}>
      <div className={'flex w-full flex-row gap-2'}>
        ...
        <motion.div
          layout
          ref={containerRef}
          className='selected no-scrollbar flex h-12 w-[fit-content] items-center overflow-x-scroll scroll-smooth rounded-md border border-solid border-white/10 p-2'
        >
          <motion.div layout className='flex items-center gap-2'>
            {selected?.map((item) => (
              <Tag
                name={item?.key}
                key={item?.key}
                className={'bg-white/10 backdrop-blur-lg shadow'}
              >
        ...
        {tags?.length > selected?.length && (
          <div className='flex w-[fit-content] flex-wrap gap-2 rounded-md bg-transparent p-2'>
            {tags
...
export const Tag = ({ children, className, name, onClick }: TagProps) => {
  return (
    <motion.div
      ...
      onClick={onClick}
      className={(
        `cursor-pointer rounded-md bg-transparent px-2 py-1 text-sm hover:bg-white/10 ${className}` 
      )}
    >
      {children}
    </motion.div>
